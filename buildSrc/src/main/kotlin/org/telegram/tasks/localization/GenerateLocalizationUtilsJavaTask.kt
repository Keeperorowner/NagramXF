    ...
    private fun generateJava(
        ...
        packageDir.resolve("LocalizationUtils.java")
            ...
            .use { java ->
                ...
                val plainLanguages = sortedSetOf<String>()

                for (tag in languageTags) {
                    if (!tag.contains('-')) {
                        plainLanguages.add(tag)
                        java.append("            case \"")
                        java.append(escapeJavaString(tag))
                        java.append("\": return \"")
                        java.append(getLocalizationAssetName(tag))
                ...
                // Region-tagged localizations (zh-CN, ja-JP, ...) are also reachable
                // by their bare language code: remote langpacks resolve to Locale("zh"),
                // Locale("ja") etc. First tag in sorted order wins per language, so
                // "zh" maps to zh-CN while zh-TW stays reachable via exact match above.
                val fallbackByLanguage = sortedMapOf<String, String>()

                for (tag in languageTags) {
                    if (tag.contains('-')) {
                        val language = tag.substringBefore('-')
                        if (!plainLanguages.contains(language) && !fallbackByLanguage.containsKey(language)) {
